...
MartinPositive::MartinPositive() : Fractal()
{
	...
	Defaults.Set(1, 1000000, 4, 45, 2, -300, 1);
...
void MartinPositive::RenderSS(int hstart, int hend)
{
}
...
void MartinPositive::Render(int hstart, int hend)
{
	...
	int delta = Fast::Floor((long double)max_iterations / pp->ColourCount);
	int count = 0;
	int index = 0;
...
void MartinPositive::ResetView()
{
	...
	int x_max = Fast::Floor((double)Width / (2 * Var.d));

	int y_min = Fast::Floor(-(double)Height / (2 * Var.d));
	int y_max = Fast::Floor((double)Height / (2 * Var.d));

	SetView(x_min, x_max, y_min, y_max);
}


std::wstring MartinPositive::GetParameters()
{
...
std::wstring MartinPositive::Description()
{
