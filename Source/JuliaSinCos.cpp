...
JuliaSinCos::JuliaSinCos() : Fractal()
{
	...
	Defaults.Set(1, 250, 6, -0.9, -0.8, 0, 1000, 0);
...
{
	return L"Julia Sin(z) + Cos(z): r " + Formatting::LDToStr(Var.a) + L"; i " + Formatting::LDToStr(Var.b) + L", x " + Formatting::LDToStr(xmin) + L" <-> " + Formatting::LDToStr(xmax) + L" y " + Formatting::LDToStr(ymin) + L" <-> " + Formatting::LDToStr(ymax) +
...
{
	ofile << Formatting::to_utf8(L"Julia Set Sin(z) + Cos(z)\n");
