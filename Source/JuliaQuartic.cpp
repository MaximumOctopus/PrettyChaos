...
bool JuliaQuartic::MultiThreadRender(bool preview, bool super_sample, bool morph)
{
		...
		for (int x = 0; x < Width; x++)
		{
			for (int ss = 0; ss < supersamples; ss++)
			{
				...
				while (p * p + q * q <= bailout_radius && it < max_iterations)
				{
					...
					long double pow2 = (p * p + q * q) * (p * p + q * q);
...
void JuliaQuartic::RenderSSMorph(int hstart, int hend)
{
		...
		for (int x = 0; x < Width; x++)
		{
			for (int ss = 0; ss < supersamples; ss++)
			{
				...
				while (p * p + q * q <= bailout_radius && it < max_iterations)
				{
					...
					p = pow2 * std::cos(atan2pq) + vara;
					q = pow2 * std::sin(atan2pq) + varb;
...
void JuliaQuartic::RenderMorph(int hstart, int hend)
{
	long double vara = Var.a;
	long double varb = Var.b;
...
{
	return L"Julia (z^4): r " + Formatting::LDToStr(Var.a) + L"; i " + Formatting::LDToStr(Var.b) + L", x " + Formatting::LDToStr(xmin) + L" <-> " + Formatting::LDToStr(xmax) + L" y " + Formatting::LDToStr(ymin) + L" <-> " + Formatting::LDToStr(ymax) +
...
{
	ofile << Formatting::to_utf8(L"Julia Set (z^4)\n");
