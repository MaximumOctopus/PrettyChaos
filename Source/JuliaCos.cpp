...
JuliaCos::JuliaCos() : Fractal()
{
	...
	Defaults.Set(1, 1000, 4, -1.6, 0.7061, 0, 1000, 0);
...
bool JuliaCos::MultiThreadRender(bool preview, bool super_sample, bool morph)
{
...
void JuliaCos::RenderMorph(int hstart, int hend)
{
...
void JuliaCos::RenderSSMorph(int hstart, int hend)
{
	long double vara = Var.a;
	long double varb = Var.b;
...
{
	return L"Julia Cos(z): r " + Formatting::LDToStr(Var.a) + L"; i " + Formatting::LDToStr(Var.b) + L", x " + Formatting::LDToStr(xmin) + L" <-> " + Formatting::LDToStr(xmax) + L" y " + Formatting::LDToStr(ymin) + L" <-> " + Formatting::LDToStr(ymax) +
...
{
	ofile << Formatting::to_utf8(L"Julia Set Cos(z)\n");
